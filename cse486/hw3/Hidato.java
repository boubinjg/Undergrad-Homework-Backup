...
public class Hidato
{
		...
		public boolean canExist(int index, int number, int[][] map)
		{
			...
			for(int i = 0; i<map[0].length; i++)
				for(int j = 0; j<map[0].length; j++)
				{
					...
					if(map[i][j] == (number-1)  || map[i][j] == (number +1))
					{
	...
	public void solve(HidatoBoard board)
	{
		...
		else
		{
			...
			List<Integer> emptyList = new ArrayList<Integer>(board.empty);
			
			//next available board square
			//int targPlacement = it.next();
			int targPlacement = emptyList.get(j);
	...
	public boolean valid(HidatoBoard board)
	{
		...
		for(int i = 0; i<size*size; i++)
			for(int j = 0; j<size*size; j++)
			{
				...
				if(Math.abs(map[r1][c1]-map[r2][c2]) == 1  && !(map[r1][c1] == 0 || map[r2][c2] == 0)) 
				{
