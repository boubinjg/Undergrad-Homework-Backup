...
double median(const std::vector<int>& numbers) {
	...
	if((ret.size())%2==0)
		return (double)(ret[ret.size()/2]+ret[ret.size()/2-1])/2;
	else
...
int mode(const std::vector<int>& numbers) {
	std::vector<int> ret(numbers);
	std::sort(ret.begin(),ret.end());
	int count=0, highestCount=0, mode = 0, prevNum = 0;
...
bool contains(const std::vector<int>& list1, const std::vector<int>& list2) {
	std::vector<int> sto(list1);
	for(std::vector<int>::const_iterator it = list2.begin(); it != list2.end(); it++)
	{
		if(std::find(sto.begin(),sto.end(), *it) != sto.end())
		{
			sto.erase(std::find(sto.begin(),sto.end(),*it));
		}
		else
			return false;
	}
...
std::vector<int> readVector(std::istream& is) {
	...
	for(std::istream_iterator<int> it (is); *it!=-1; it++)
	{
		ret.push_back(*it);
	}
	return ret;
...
/**
   erints each number (followed by a space) in the given list to the
   given output stream.
